...
const getFee = async (req, res) => {
  try {
    ...
    res.json({ status: 'success', message: 'Fee fetched', data: { fee, payments } })
  } catch (error) {
    res.status(500).json({ status: 'error', message: error.message })
  }
}
...
const getOverdueFees = async (req, res) => {
  try {
    ...
    res.json({ status: 'success', message: 'Overdue fees fetched', data: fees })
  } catch (error) {
    res.status(500).json({ status: 'error', message: error.message })
  }
}
...
const recordPayment = async (req, res) => {
  try {
    ...
    if (!studentFeeId || !amountPaid) {
      return res.status(400).json({ status: 'error', message: 'studentFeeId and amountPaid are required' })
    }
    ...
    if (fee.status === 'paid') {
      return res.status(400).json({ status: 'error', message: 'This fee has already been fully paid' })
    }

    if (amountPaid > fee.balance) {
      return res.status(400).json({ status: 'error', message: `Amount exceeds balance of ${fee.balance}` })
    }
...
const getPayments = async (req, res) => {
  try {
    ...
    res.json({ status: 'success', message: 'Payments fetched', data: payments })
  } catch (error) {
    res.status(500).json({ status: 'error', message: error.message })
  }
}
