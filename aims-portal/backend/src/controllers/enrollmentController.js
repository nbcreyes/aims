  ...
  try {
    ...
    const enrollments = await Enrollment.find(filter)
      ...
      .populate({
        ...
        populate: {
          path: "subjectId",
          select: "name code units hasLab labFee",
        },
      })
      .sort({ enrolledAt: -1 });
  ...
  try {
    ...
    if (existing) {
      return res
        ...
        .json({
          status: "error",
          message: "Already enrolled for this semester",
        });
    }
...
const updateEnrollmentStatus = async (req, res) => {
  try {
    ...
    if (enrollment.status === "approved" && status !== "dropped") {
      return res
        .status(400)
        .json({ status: "error", message: "Enrollment is already approved" });
    }
    ...
    await notify(
      ...
      `Enrollment ${status.charAt(0).toUpperCase() + status.slice(1)}`,
      `Your enrollment for this semester has been ${status}.`,
    );
    ...
    if (status === "approved") {
      ...
      if (!existingFee) {
        const enrolledSubjects = enrollment.subjects
          .map((s) => s.subjectId)
          .filter(Boolean);
        const feeData = computeStudentFees(
