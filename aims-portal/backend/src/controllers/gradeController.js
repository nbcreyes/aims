...
const getPreviousGrades = async (studentId, scheduleId) => {
  ...
  const map = {}
  grades.forEach(g => { map[g.term] = g.cumulativeGrade })
  return map
}
...
const getGradesheet = async (req, res) => {
  try {
    const { scheduleId, term } = req.query
    if (!scheduleId || !term) {
      return res.status(400).json({ status: 'error', message: 'scheduleId and term are required' })
    }

    const students = await getEnrolledStudents(scheduleId)

    const sheet = await Promise.all(students.map(async (student) => {
      let grade = await TermGrade.findOne({ studentId: student._id, scheduleId, term })
      if (!grade) {
        grade = {
          studentId: student,
          scheduleId,
...
const upsertGrade = async (req, res) => {
  try {
    const {
      studentId, scheduleId, semesterId, term,
      quizScores, activityScores, examScore, examMaxScore
    } = req.body

    if (!studentId || !scheduleId || !semesterId || !term) {
      return res.status(400).json({ status: 'error', message: 'studentId, scheduleId, semesterId, and term are required' })
    }
    ...
    if (term === 'prelim') {
      const midterm = await TermGrade.findOne({ studentId, scheduleId, term: 'midterm' })
      if (midterm) {
        const updatedPrevious = { prelim: computed.cumulativeGrade }
        const midtermComputed = computeGradeForTerm(midterm, 'midterm', updatedPrevious)
        midterm.classStanding = midtermComputed.classStanding
        midterm.cumulativeGrade = midtermComputed.cumulativeGrade
        await midterm.save()

        const finals = await TermGrade.findOne({ studentId, scheduleId, term: 'finals' })
        if (finals) {
...
const publishGrades = async (req, res) => {
  try {
    ...
    res.json({ status: 'success', message: `${term} grades published`, data: null })
  } catch (error) {
    res.status(500).json({ status: 'error', message: error.message })
  }
}
...
const unpublishGrades = async (req, res) => {
  try {
    ...
    res.json({ status: 'success', message: `${term} grades unpublished`, data: null })
  } catch (error) {
    res.status(500).json({ status: 'error', message: error.message })
  }
}
