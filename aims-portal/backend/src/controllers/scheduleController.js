...
const createSchedule = async (req, res) => {
  try {
    ...
    const teacherConflict = await checkTimeConflict({
      teacherId,
      day,
      timeStart,
      timeEnd,
    ...
    if (teacherConflict) {
      return res
        ...
        .json({
          status: "error",
          message: "Teacher has a conflicting schedule on this day and time",
        });
    }

    // Check room conflict — same room, same day, overlapping time
    const roomConflict = await checkTimeConflict({
      room,
      day,
      timeStart,
      timeEnd,
    ...
    if (roomConflict) {
      return res
        ...
        .json({
          status: "error",
          message: "Room is already occupied at this day and time",
        });
    }

    // Check section conflict — same section, same day, overlapping time
    const sectionConflict = await checkTimeConflict({
      sectionId,
      day,
      timeStart,
      timeEnd,
    ...
    if (sectionConflict) {
      return res
        ...
        .json({
          status: "error",
          message: "Section already has a class at this day and time",
        });
    }
