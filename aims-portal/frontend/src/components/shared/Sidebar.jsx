...
export default function Sidebar() {
  ...
  const mainLinks = links.filter(
    (l) => l.label !== "Profile" && l.label !== "Change Password",
  );
      ...
      <div className="px-4 py-4 border-b border-gray-100 flex items-center justify-between">
        {!collapsed && (
          <div>
            ...
            <span
              className={`text-xs px-1.5 py-0.5 rounded font-medium capitalize mt-0.5 inline-block ${ROLE_COLORS[user?.role] || "bg-gray-100 text-gray-600"}`}
            >
              {user?.role}
            </span>
          </div>
