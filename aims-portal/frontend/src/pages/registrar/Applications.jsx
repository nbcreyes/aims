  ...
  const handleStatusUpdate = async () => {
    ...
    try {
      const res = await api.put(
        `/applications/${selected._id}/status`,
        statusForm,
      );
    ...
    <DashboardLayout>
      <div className="p-6">
        ...
        <div className="flex gap-2 mb-4">
          {["all", "pending", "under_review", "accepted", "rejected"].map(
            (s) => (
              <button
                key={s}
                onClick={() => setFilter(s)}
              ...
              {applications.length === 0 ? (
                ...
                applications.map((app) => (
                  <tr key={app._id} className="hover:bg-gray-50">
                    ...
                    <td className="px-4 py-3 text-gray-600">
                      {app.semesterId?.schoolYear} {app.semesterId?.term}
                    </td>
          ...
          <div className="bg-white border border-gray-200 rounded-lg p-6">
            <div className="flex items-center justify-between mb-4">
              <h2 className="text-sm font-semibold text-gray-700">
                Review Application — {selected.name}
              </h2>
              <button
            ...
            <div className="grid grid-cols-2 gap-4 text-sm mb-4">
              ...
              <div>
                ...
                <span className="text-gray-800">
                  {selected.programId?.name}
                </span>
              </div>
              <div>
                <span className="text-gray-500">Semester:</span>{" "}
                <span className="text-gray-800">
                  {selected.semesterId?.schoolYear} {selected.semesterId?.term}
                </span>
              </div>
              <div>
                ...
                <div className="flex flex-wrap gap-2">
                  {selected.documents.map((doc, i) => (
                    <a
                      ...
                      target="_blank"
                      rel="noreferrer"
                      className="text-xs text-blue-600 hover:underline border border-blue-200 px-2 py-1 rounded capitalize"
                    >
                      {doc.docType}
                    </a>
                  ))}
            ...
            {selected.status === "accepted" && (
              <p className="text-sm text-green-700 bg-green-50 px-4 py-2 rounded border border-green-200">
                This application has been accepted and a student account has
                been created.
              </p>
            )}
          </div>
